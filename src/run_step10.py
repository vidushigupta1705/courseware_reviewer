...
from diagram_generator import generate_diagrams_for_recommendations
from table_code_analysis import analyze_tables_and_code
from deterministic_checks import run_all_checks_step10
...
def process_file(input_file: Path, resume=True):
    ...
    if current_stage == "units_done":
        state = analyze_diagram_recommendations(state)
        save_checkpoint(state, "diagram_recommend_done")
        current_stage = "diagram_recommend_done"

    if current_stage == "diagram_recommend_done":
        state = generate_diagrams_for_recommendations(state)
        save_checkpoint(state, "diagram_generate_done")
        current_stage = "diagram_generate_done"

    if current_stage == "diagram_generate_done":
        state = analyze_tables_and_code(state)
        save_checkpoint(state, "table_code_done")
        current_stage = "table_code_done"

    if current_stage == "table_code_done":
    ...
    if current_stage == "accuracy_done":
        state = run_all_checks_step10(state)
    ...
    json_path = INTERMEDIATE_JSON_DIR / f"{state.base_filename}_step10_state.json"
    ...
    print(f"Tables detected: {len(getattr(state, 'tables', []))}")
    print(f"Generated diagrams: {len(getattr(state, 'generated_diagrams', []))}")
