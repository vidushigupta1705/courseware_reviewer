...
def process_file(input_file: Path, resume=True):
    ...
    if current_stage == "diagram_recommend_done":
        state = build_visual_specs(state)
        save_checkpoint(state, "visual_spec_done")
        current_stage = "visual_spec_done"

    if current_stage == "visual_spec_done":
        state = render_advanced_visuals(state)
        save_checkpoint(state, "advanced_visual_done")
        current_stage = "advanced_visual_done"

    if current_stage == "advanced_visual_done":
    ...
    if current_stage == "accuracy_done":
        state = run_all_checks_step11(state)
