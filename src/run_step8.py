...
from deterministic_checks import run_all_checks_step8
...
def process_file(input_file: Path, resume=True):
    ...
    if current_stage == "units_done":
        ...
        save_checkpoint(state, "diagram_done")
        current_stage = "diagram_done"

    if current_stage == "diagram_done":
    ...
    if current_stage == "accuracy_done":
        state = run_all_checks_step8(state)
    ...
    json_path = INTERMEDIATE_JSON_DIR / f"{state.base_filename}_step8_state.json"
    ...
    print(f"Diagram recommendations: {len(getattr(state, 'diagram_recommendations', []))}")
