...
from deterministic_checks import run_all_checks_step4
...
def process_file(input_file: Path):
    ...
    state = run_all_checks_step4(state)
    ...
    json_path = INTERMEDIATE_JSON_DIR / f"{state.base_filename}_step4_state.json"
    ...
    print(f"Duplicate findings: {len(getattr(state, 'duplicate_findings', []))}")
